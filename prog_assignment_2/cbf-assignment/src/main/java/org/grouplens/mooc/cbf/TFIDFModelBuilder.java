    ...
    public TFIDFModel get() {
        ...
        for (long item: items) {
			...
			for (String tag : tags)
			{
				...
				{
					double count = work.get(tagId);
					work.set(tagId, count + 1d);
				} else
				{
					work.set(tagId, 1.0d);
				}
			...
			{
				long uniqueTagId = tagIds.get(uniqueTag);

				if (docFreq.containsKey(uniqueTagId))
				{
					double tagCount = docFreq.get(uniqueTagId);
					docFreq.set(uniqueTagId, tagCount + 1d);
				} else
				{
					docFreq.set(uniqueTagId, 1d);
				}
			}
        ...
        for (Map.Entry<Long,MutableSparseVector> entry: itemVectors.entrySet()) {
			...
			for (VectorEntry e : tv.fast())
			{
				...
				double idf = docFreq.get(e.getKey());
				tv.set(e.getKey(), tf * idf);
			}

			// Normalize the TF-IDF vector to be a unit vector
            // HINT The method tv.norm() will give you the Euclidian length of the vector
			double euclidianLength = tv.norm();

			for (VectorEntry e : tv.fast())
			{
				tv.set(e.getKey(), e.getValue() / euclidianLength);
			}
