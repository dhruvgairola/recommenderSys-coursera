...
public class TFIDFItemScorer extends AbstractItemScorer {
    ...
    public void score(long user, @Nonnull MutableSparseVector output) {
        ...
        for (VectorEntry e: output.fast(VectorEntry.State.EITHER)) {
			...
			double sim = cosine.similarity(iv, userVector);
    ...
    private SparseVector makeUserVector(long user) {
		...
		// calcUnweighted(profile, userRatings);
		calcWeighted(profile, userRatings);
	...
	private void calcUnweighted(MutableSparseVector profile, List<Rating> userRatings)
	{
	...
	private void calcWeighted(MutableSparseVector profile, List<Rating> userRatings)
	{
		...
		{
			Preference p = r.getPreference();
			totalRatings += p.getValue();
			count++;
		}

		double mean = totalRatings / count;

		for (Rating r : userRatings)
		{
			Preference p = r.getPreference();
			double weight = p.getValue() - mean;
			MutableSparseVector item = model.getItemVector(p.getItemId()).mutableCopy();
			item.multiply(weight);
			profile.add(item);
        }
    }
}
